...
import { TreeView } from '../../sharedCommonComponents/components/TreeView';
...
export const DiseaseEditFormPage = (props: DiseaseEditFormPageProps) => {
    ...
    const { icdCode: matchedIcdCode } = useParams();
    ...
    const store = async () => {
        ...
        try {
            setIsStoring(true);
            await apiClient.instance!.put(`/api/diseases/${disease.icd11Code}`, {}, disease);
    ...
    if(!matchedIcdCode) {
        return (<h3>{resolveText("NoIdSpecified")}</h3>);
    }
