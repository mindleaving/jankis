    ...
    return (
        <div className="listFormControl">
            {props.items.map(item => 
                <Badge className="itemListBadge" variant="info" key={props.idFunc(item)}>
                    {props.displayFunc(item)} <Badge pill variant="danger" onClick={() => props.removeItem(item)}>X</Badge> 
                </Badge>
            )}
        </div>
    );
