...
interface BedOccupancyEditPageProps {}

export const BedOccupancyEditPage = (props: BedOccupancyEditPageProps) => {

    const location = useLocation();
    const params: BedOccupancyParams = useParams();
    const isNew = location.pathname.toLowerCase().startsWith('/create');
    const matchedId = params.occupancyId;
    const matchedDepartmentId = params.departmentId;
    const matchedRoomId = params.roomId;
    const matchedBedPosition = params.bedPosition;
    if(!isNew && !matchedId) {
    ...
    const store = async (e: FormEvent) => {
        e.preventDefault();
        setIsStoring(true);
        await buildAndStoreObject<Models.BedOccupancy>(
